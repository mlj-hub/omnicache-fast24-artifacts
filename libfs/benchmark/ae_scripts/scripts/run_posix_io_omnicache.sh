...
RUN() {
        ...
        numactl --physcpubind=0-31 --membind=0 $MICROBENCH/build/test_smart_cache_posix_hybrid $2 4096 $1 $3 $1 | tee $output/result.txt

	$LIBFS/crfsexit

        #$MICROBENCH/build/test_smart_cache_posix_hybrid $2 4096 $1 $3 $1 
        # $MICROBENCH/build/test_smart_cache_posix_hybrid $2 4096 $1 $3 $1 | tee $output/result.txt 
        #sudo perf record -F 99 $MICROBENCH/build/test_smart_cache_posix_hybrid $2 4096 $1 $3 $1 #&> $output/result.txt 
