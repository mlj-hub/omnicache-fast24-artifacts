...
$BASE/appbench/leveldb/ae_scripts/scripts/kill_leveldb_omnicache.sh &
echo "BEGIN TO RUN"
...
RUN() {
        ...
        numactl --physcpubind=0-31 --membind=0  $LEVELDB/db_bench_omnicache --db=$DBPATH --benchmarks=$BENCHMARK --use_existing_db=0 --num=$KEYS --value_size=$2 --threads=$1 --open_files=256 | tee $output/result.txt
        export LD_PRELOAD=""
