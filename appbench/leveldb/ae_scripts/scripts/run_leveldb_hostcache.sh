...
RUN() {
        ...
        numactl --physcpubind=0-31 --membind=0 $LEVELDB/db_bench_hostcache --db=$DBPATH --benchmarks=$BENCHMARK --use_existing_db=0 --num=$KEYS --value_size=$2 --threads=$1 --open_files=256 | tee $output/result.txt
        export LD_PRELOAD=""
